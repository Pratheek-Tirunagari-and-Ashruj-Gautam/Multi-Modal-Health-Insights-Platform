...
class ChestXRayDataset(Dataset):
    def __init__(self, base_dir):
        ...
        # Extract actual image paths
        train_dir = os.path.join(base_dir, 'train')
        self.image_files = [f for f in os.listdir(train_dir) if f.endswith(('.jpg', '.png'))]
        print(f"Found {len(self.image_files)} image files in train directory")
    ...
    def __getitem__(self, idx):
        try:
            ...
            # Use the actual image file from our train directory
            img_filename = self.image_files[idx % len(self.image_files)]  # Cycle through available images
