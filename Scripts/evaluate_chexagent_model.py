...
def load_test_data(data_path):
    ...
    # Load metadata file
    metadata_path = "data/processed/metadata/test_metadata.csv"
...
def load_model(model_path, device):
    ...
    try:
        # Create a simple model that matches the saved architecture
        class SimpleModel(nn.Module):
            def __init__(self, num_classes=13):
                super().__init__()
                self.linear = nn.Linear(1024, num_classes)  # DenseNet121 feature size
        ...
        model.feature_extractor = nn.Sequential(*list(feature_extractor.features.children()))
...
def main():
    ...
    test_dataset = ChestXrayDataset(image_paths, labels)
